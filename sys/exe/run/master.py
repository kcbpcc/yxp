...
############################################"PXY® PreciseXceleratedYield Pvt Ltd™############################################
    combined_df[numeric_columns] = combined_df[numeric_columns].round(1)        # Filter combined_df
    mis_filtered_df = combined_df[(combined_df['qty'] < 0)] # & (combined_df['PnL%'] > 0)
    # Filter combined_df for rows where 'qty' is greater than 0
    combined_df_negative_qty = combined_df[(combined_df['qty'] < 0) & (combined_df['source'] == 'positions') & (combined_df['product'] == 'MIS')]
    # Calculate and print the sum of 'PnL' values and its total 'PnL%' for rows where 'qty' is greater than 0
    total_nPnL = combined_df_negative_qty['PnL'].sum()
    total_nPnL_percentage = (total_PnL / combined_df_positive_qty['Invested'].sum()) * 100
    # Calculate and print the sum of 'dPnL' values and its total 'dPnL%' for rows where 'qty' is greater than 0
    total_ndPnL = combined_df_negative_qty['dPnL'].sum()
    total_ndPnL_percentage = (total_dPnL / combined_df_negative_qty['Invested'].sum()) * 100
############################################"PXY® PreciseXceleratedYield Pvt Ltd™############################################
    import pandas as pd
    ...
    ############################################"PXY® PreciseXceleratedYield Pvt Ltd™############################################
    # Assuming NIFTY['Day_Change_%'] is a Pandas Series
    nPrecise = max(1.4, (1 + (NIFTY['Day_Change_%'] * 2.4).round(1)).max())
    nXlratd = max(1.4, (2 + (NIFTY['Day_Change_%'] * 3.4).round(1)).max())
    nYield = max(1.4, (3 + (NIFTY['Day_Change_%'] * 4.4).round(1)).max())
    nconditions_pxy = [(mktpxy == 'Bull') | (mktpxy == 'Buy'), (mktpxy == 'Sell'), (mktpxy == 'Bear')]
    nchoices_pxy = ['Yield', 'Xlratd', 'Precise']
    nPXY = np.select(conditions_pxy, choices_pxy, default='Yield')
    ############################################"PXY® PreciseXceleratedYield Pvt Ltd™############################################
    ...
    print(left_aligned_format.format(f"Total Day ndPnL {round(total_dPnL, 2)}"), end="")
    print(right_aligned_format.format(f"Total Day dPnL {round(total_dPnL, 2)}"))

    print(left_aligned_format.format(f"Total Day ndPnL% {round(total_dPnL_percentage, 2)}"), end="")
    print(right_aligned_format.format(f"Total Day dPnL% {round(total_dPnL_percentage, 2)}"))

    print(left_aligned_format.format(f"Precise Check (nPrecise): {round(Precise, 2)}"), end="")
    print(right_aligned_format.format(f"Precise Check (Precise): {round(Precise, 2)}"))

    print(left_aligned_format.format(f"Xcelerated Check (nXlratd): {round(Xlratd, 2)}"), end="")
    print(right_aligned_format.format(f"Xcelerated Check (Xlratd): {round(Xlratd, 2)}"))

    print(left_aligned_format.format(f"Yield Check (nYield): {round(Yield, 2)}"), end="")
    print(right_aligned_format.format(f"Yield Check (Yield): {round(Yield, 2)}"))
