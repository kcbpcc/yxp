...
try:
    ...
    Precise = max(1.4, (0 + (NIFTY['strength'] * 4).round(1).max()))
    Xlratd = max(1.4, (1 + (NIFTY['strength'] * 5).round(1).max()))
    Yield = max(1.4, (2 + (NIFTY['strength'] * 6).round(1).max()))
    ...
    if any(item in mktpxy for item in ['Sell', 'Bear', 'Buy', 'Bull', 'None']):  # Check if mktpxy is one of the specified values
        ...
        print(right_aligned_format.format(f"Yield:{BRIGHT_GREEN if Yield > 4.4 else BRIGHT_RED}{round(Yield, 2)}{RESET}"))
