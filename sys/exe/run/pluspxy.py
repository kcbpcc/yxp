...
try:
    ...
    if any(item in mktpxy for item in ['Sell', 'Bear', 'Buy', 'Bull', 'None']):  # Check if mktpxy is one of the specified values
        ...
        print(left_aligned_format.format(f"{'🔴🔴🔴' if mktpxy == 'Bear' else '🟢🟢🟢' if mktpxy == 'Bull' else '🔴🛬⤵️' if mktpxy == 'Sell' else '🟢🛫⤴️' if mktpxy == 'Buy' else 'Unknown'} {mktpxy}{RESET}"), end="")
        print(right_aligned_format.format(f"@PnL%:{BRIGHT_GREEN if total_PnL_percentage >= 0 else BRIGHT_RED}{round(total_PnL_percentage, 2)}{RESET}"))
