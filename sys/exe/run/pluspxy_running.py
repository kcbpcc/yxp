...
try:
    ...
    filtered_df = combined_df[(combined_df['qty'] > 0)] # & (combined_df['PnL%'] > 0)
    ...
    pxy_df = filtered_df.copy()[['source', 'qty','average_price', 'close', 'ltp', 'open', 'high','low', 'key','dPnL%','PnL','PnL%_H', 'PnL%']]
    pxy_df['Pre'] = Precise
    ...
    EXE_df = pxy_df[['source', 'key', 'qty','avg','close', 'ltp', 'open', 'high','low','dPnL%','PXY','PnL%','PnL']]
    PRINT_df = pxy_df[['source', 'key', 'qty','avg','ltp','dPnL%','PXY','PnL%','PnL']]
    # Sort the DataFrame by 'PnL%' in ascending order
    # Assuming you have a DataFrame named PRINT_df
    PRINT_df_sorted = PRINT_df[PRINT_df['PnL%'] > 0].sort_values(by='PnL%', ascending=True)
    SILVER = "\033[97m"
    UNDERLINE = "\033[4m"
    RESET = "\033[0m"
        ...
        try:
            for index, row in EXE_df.iterrows():
                ...
                if (
                    ...
                    elif row['source'] == 'positions' and (row['PnL%'] > row['PXY']):
                        # Print the row before placing the order
                        print(row)
