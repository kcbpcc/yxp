...
if decision == "YES" and mktpxy in ['Buy', 'Bull', 'Bear', 'Sell']:
    ...
    def transact(dct):
        try:
            ...
            market_sentiment = smktchk(yahoo_symbol, intervals[0])
            ...
            if order_id:
                logging.info(f"BUY {order_id} placed for {yahoo_symbol} successfully")
            else:
                logging.error(f"Unable to place order for {yahoo_symbol}")

        except Exception as e:
            logging.error(f"{str(e)} while placing order for {yahoo_symbol}")
