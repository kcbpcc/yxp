...
try:
    ...
    total_PnL_percentage_mis_buy = (total_PnL / combined_df_positive_qty['Invested'].sum()) * 100 if (combined_df['product'] == "MIS").any() and combined_df_positive_qty['qty'].sum() > 0 else None
    total_PnL_percentage_mis_sell = (total_PnL / combined_df_positive_qty['Invested'].sum()) * 100 if (combined_df['product'] == "MIS").any() and combined_df_positive_qty['qty'].sum() < 0 else None
   ...
   # Sort the DataFrame by 'PnL' in ascending order
    ...
    if any(item in mktpxy for item in ['Sell', 'Bear', 'Buy', 'Bull', 'None']):  # Check if mktpxy is one of the specified values
        ...
        print(left_aligned_format.format(f"MISsell:{BRIGHT_YELLOW}{round(total_PnL_percentage_mis_sell, 0)}{RESET}"), end="")
        print(right_aligned_format.format(f"MISbuy-:{BRIGHT_YELLOW}{round(total_PnL_percentage_mis_buy, 0)}{RESET}"))
