...
try:
    ...
    if any(item in mktpxy for item in ['Sell', 'Bear', 'Buy', 'Bull', 'None']):  # Check if mktpxy is one of the specified values
            ...
            for index, row in EXE_df.iterrows():
                ...
                if (
                    ...
                    if (
                        ...
                        row['product'] == 'CNC' and
                        row['PnL%'] > 1.4 and 
                        ((row['PnL%'] < ((row['pxy'])*1) and row['PnL%_H'] > ((row['pxy'])*1)) or (row['PnL%'] > TIMPXY))
        ...
        print(right_aligned_format.format(f"Power:{BRIGHT_GREEN if (Pr > 0.5) and isinstance(Pr, (int, float)) else BRIGHT_RED}{'---' if not isinstance(Pr, (int, float)) else round(Pr, 2)}{RESET}"))
