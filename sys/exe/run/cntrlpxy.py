...
try:
   ...
   # Sort the DataFrame by 'PnL' in ascending order
    ...
    if any(item in mktpxy for item in ['Sell', 'Bear', 'Buy', 'Bull', 'None']):  # Check if mktpxy is one of the specified values
        ...
        print(left_aligned_format.format(f"MISsellPnL: {round(total_PnL_percentage_mis_sell, 0)}"), end="")
        print(right_aligned_format.format(f"MISbuyPnL: {round(total_PnL_percentage_mis_buy, 0)}"))
